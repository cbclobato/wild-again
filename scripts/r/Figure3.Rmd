...
counts <- asv_prev_rel_taxo_gen_obs_core_bmk %>% 
  dplyr::count(Domain = "Bacteria")
...
p_core_flex1_land <-
  ...
  geom_point(aes(color = bmk_core_asv), size = size_p, alpha = 0.7) + 
  ggrepel::geom_text_repel(data = bmks, max.overlaps = 20,
                           aes(asv_rel_p,
                               asv_prev_p,
                               color = bmk_core_asv,
                               label = ASV),
                           size = size_l, show.legend = F) +
  geom_text(data = counts, 
            aes(x = 9.5, y = 5, label = n, fontface = "bold"), 
            size = size_l) +
  ...
  theme(
    ...
    axis.title.x = element_text(size = size_ti),
    axis.title.y = element_blank(), 
    axis.text = element_text(size = size_tx),
    axis.line = element_blank(),
    legend.text = element_text(size = size_ti),
    legend.position = c(0.2, 0.95), 
    legend.direction = "vertical",
    legend.background = element_rect(fill = "transparent", color = "transparent"),
    legend.margin = margin(0, 0, 0, 0),
    panel.spacing = unit(0.1, "lines")) +
  scale_x_log10(breaks = c(10, 1, 0.1, 0.01, 0.001), 
...
p_core_flex1_sel <-
  ...
  ggrepel::geom_text_repel(data = bmks, max.overlaps = 20,
                           aes(asv_rel_p,
                               ...
                               label = ASV),
                           size = size_l, show.legend = F) +
  geom_text(data = counts, aes(x = 9.5, y = 5, label = n, fontface = "bold"), 
            size = size_l) +
  geom_hline(yintercept = 75, linetype = "dashed", color = "black") +
  geom_vline(xintercept = 0.1, linetype = "dotted", color = "black", show.legend = T) +
  theme_classic() +
...
p_core_flex1_all <- 
  ggplot(asv_prev_rel_taxo_gen_obs_core_all, 
         aes(x = asv_rel_p, y = asv_prev_p)) +
  geom_point(aes(color = core_asv), 
             size = 3, 
             alpha = 0.7) + 
  ggrepel::geom_text_repel(data = subset(asv_prev_rel_taxo_gen_obs_core_all, 
                                         core_asv == "Core"),
                           max.overlaps = 20,
                           aes(asv_rel_p,
                               asv_prev_p,
                           ...
                           size = 3,
                           show.legend = F) +
  geom_text(data = counts, aes(x = 9.5, y = 5, label = n,  fontface = "bold"), size = 4) +
  geom_hline(yintercept = 75, linetype = "dashed", color = "black") +
  geom_vline(xintercept = 0.1, linetype = "dotted", color = "black", show.legend = T) +
  theme_classic() +
  theme(
  ...
  scale_color_manual(values = color_core)	+
  labs(x = "Relative abundance [%]", y = "Prevalence [%]", color = ""); p_core_flex1_all
```
...
# save(p_core_flex1_lsci, file = "outputs/r/fig_3/core_flex1-final.RData")
```
...
p_core_flex3_land <-
  ...
  theme(axis.title = element_blank(),
        ...
        legend.text = element_text(size = size_sb),
  ...
  guides(fill = guide_legend(ncol = 1)); p_core_flex3_land
```
...
p_core_flex3_all <-
  ...
  coord_fixed(ratio = 0.05) +
  guides(fill = guide_legend(ncol = 1)); p_core_flex3_all
```
...
p_core_flex2_lsci <- cowplot::plot_grid(p_core_flex2_land,
                                        ...
                                        p_core_flex3_ib,
                                        nrow = 1,
                                        rel_widths = c(0.1, 0.25, 0.1, 0.25,
                                                       0.1, 0.25, 0.1, 0.25),
                                        align = "h",
                                        greedy = F); p_core_flex2_lsci

# save(p_core_flex2_lsci, file = "outputs/r/fig_3/core_flex2-final.RData")
```
...
p_core_flex3_lsci <-
  ...
  labs(x = "",
       y = "") +
  coord_flip(); p_core_flex3_lsci

# save(p_core_flex3_lsci, file = "outputs/r/fig_3/core_flex3-final.RData")
```
...
p_core_flex_lsci_f123 <- cowplot::plot_grid(p_core_flex2_lsci,
                                            ...
                                            label_size = 15,
                                            label_x = -0.01,
                                            label_y = c(1.13, 1.09, 1.05),
                                            ncol = 1,
                                            rel_widths = c(1, 1, 1),
                                            rel_heights = c(0.88, 0.52, 0.9)); p_core_flex_lsci_f123

Fig3 <- p_core_flex_lsci_f123 +
  theme(plot.margin = margin(1,0,0,0.25, "cm")) +
  geom_text(aes(x = rep(-0.004, 3),
                y = c(0.825, 0.52, 0.2)),
            label = c("Relative abundance [%]", 
                      "Ft.importance", 
                      "Prevalence [%]"),
            size = 3.25,
            angle = 90) +
  geom_text(aes(x = c(0.27, 0.77),
                y = rep(1.03, 2)),
            label = c("LOW DOMESTICATION", "HIGH DOMESTICATION"),
            size = 3.5) +
  geom_text(aes(x = c(0.15, 0.4, 0.65, 0.89), 
                y = rep(1.01, 4)),
            label = c("LANDRACES", "SELECTED LINES", "CROSS HYBRIDS", "INBRED LINES"),
            size = 3); Fig3

# save(Fig3, file = "outputs/r/fig_3/fig3-final.RData")
