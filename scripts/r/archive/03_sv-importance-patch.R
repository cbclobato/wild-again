...
#' @export
sv_importance_v2.shapviz <- function(object, kind = c("bar", "beeswarm", "both", "no"),
  ...
  imp <- .get_imp(
    shapviz::get_shap_values(object),
  ...
  if (is_bar) {
    ...
    #   groupby(Var1, Var2) %>%
    #   mutate(value = )
    # # value, color, group_by(Var1, Var2)
...
#' @export
sv_importance_v2.mshapviz <- function(object, kind = c("bar", "beeswarm", "both", "no"),
...
#   # TODO: Modify for ranking and barplot
#   
#   # Remove negative > average > sort
#   colFilter <- function(column) {
#     column[column < 0] <- NA
#     column
#   }
#   
...
#       sort(
#         colMeans(
#           apply(z, 2, function(c) colFilter(c)),
#           na.rm=TRUE),
#         decreasing = TRUE)
#       )
#   }
