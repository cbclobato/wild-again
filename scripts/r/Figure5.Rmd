...
```{r}
genes <- read.table(
  "data/metadata/PLaBase-genefreq.tsv",
  sep = "\t",
  header = TRUE,
  fill = TRUE) #1726

stat <- genes$freq %>% sum() #3186

krona <- read.table(
  "data/metadata/PLaBase-krona-mod.tsv",
  sep = "\t",
  header = TRUE,
  fill = TRUE) #3196
...
# level1 level2 level3 level4 level5 level6   code   freq 
#      2      7     41    215    908   1572   1578     17
...
df_pivot <- krona2 %>%
  select(level3, level5, level6) %>%
  as.data.frame() %>% 
  count(level3, level5, level6,  name = "Count")
  # pivot_wider(names_from = level6, values_from = level3) %>%
  # t() %>%
  # list()
# 
# print(df_pivot)
# common.ft <- Reduce(intersect, df_pivot); common.ft
# all.ft <- Reduce(union, df_pivot); all.ft
# ft <- unlist(df_pivot[[1]], recursive = F); ft
...
```{r}
#mycolors <- colorRampPalette(brewer.pal(42, "Paired"))(42)
...
p5 <- ggplot(data, 
  ...
  geom_bar(aes(x = as.factor(id),
           ...
           stat = "identity",
           alpha = 1) +
  geom_segment(data = grid_data, aes(x = end, y = 1108,
                                     xend = start, yend = 1108), 
               colour = "#a6a6a6", alpha = 0.6, size = 0.3, inherit.aes = F) +
  geom_segment(data = grid_data, aes(x = end, y = 200,
                                     xend = start, yend = 200), 
               colour = "#a6a6a6", alpha = 0.6, size = 0.3, inherit.aes = F) +
  geom_segment(data = grid_data, aes(x = end, y = 80,
                                     xend = start, yend = 80), 
               colour = "#a6a6a6", alpha = 0.6, size = 0.3, inherit.aes = F) +
    geom_segment(data = grid_data, aes(x = end, y = 30,
                                     xend = start, yend = 30), 
               colour = "#a6a6a6", alpha = 0.6, size = 0.3, inherit.aes = F) +
  geom_segment(data = grid_data, aes(x = end, y = 2,
                                     xend = start, yend = 2), 
               colour = "#a6a6a6", alpha = 0.6, size = 0.3, inherit.aes = F) +
  annotate("text", 
           x = rep(max(data$id)-1, 5), 
  ...
  geom_text(data = label_data, 
            ...
            inherit.aes = F) + # Add base line information
  labs(fill = "Class")
p5

ggsave("outputs/visual/fig_5/WGS.png",
       width = 45,
       height = 31,
       units = "cm",
       bg = "white",
